...
export default function Menu() {
  ...
  const handleAddToCart = id => {
    const findProducts = allFoods.find(food => food.id === id)
  ...
  return (
    <main className="flex flex-col bg-slate-200">
      <div className="flex relative bg-slate-900 pb-32 after:absolute after:bg-slate-200 after:top-[65%] after:left-0 after:w-full after:h-1/2 after:-skew-y-3">
        <Navbar />
        <section className="relative bg-slate-900 pt-10 pb-20 px-4 flex flex-col gap-4">
          ...
          <p data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200" className="text-slate-200 text-sm sm:text-base max-w-xl leading-loose">It does not matter what you like, how you like, and where you like it, we are always on hand to deliver according to your wish and taste</p>

        </section>
      </div>
      <section className="flex flex-col gap-6 bg-slate-200 relative z-10 pb-10 pt-5 px-4 -mt-20">
            ...
            sortedFoods.map(food => <aside key={food.id} className="flex flex-col rounded-md overflow-hidden hover:shad drop-shadow-xl bg-white max-w-[16rem] ">
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <div className="flex gap-2 md:gap-4 p-3 pt-2">
                  ...
                  <button onClick={() => handleAddToCart(food.id)} className="flex-1 bg-orange-400 text-white py-1 px-6 rounded-full border border-orange-400 hover:bg-transparent hover:text-orange-400 whitespace-nowrap w-32 ">Add to Cart</button>
                </div>
              </div>
            </aside>)
          }
